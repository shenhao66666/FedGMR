...
class LSTM(torch.nn.Module):
    ...
    #新增
    def forward(self, x, exogenous_data=None, device="cpu", y_hist=None, return_features=False):
        if not self.is_matrix:
            x = x.view([x.size(0), -1, x.size(1)])
        else:
            x = x.reshape(x.size(0), x.size(1), x.size(2))
        ...
        out, (_, _) = self.lstm(x, (h0.detach(), c0.detach()))
        features = out[:, -1, :]  # 取最后时刻的隐藏状态作为特征

        if exogenous_data is not None and self.is_matrix:
            features = torch.cat((features, exogenous_data), dim=1)

        out = self.MLP_layers(features)
        if return_features:
            return out, features
        return out
