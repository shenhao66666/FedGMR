...
from ml.fl.server.aggregation.aggregate import fedavg_aggregate, median_aggregate, fednova_aggregate, \
    fedadagrad_aggregate, simple_aggregate, fedyogi_aggregate, fedadam_aggregate, fedavgm_aggregate, aggregate_with_ddpg
...
class Aggregator:
    ...
    def __repr__(self):
        ...
        elif self.alg == "gate_aware_fedamp":
            expert_threshold = self.params.get('expert_threshold', 0.15) if self.params else 0.15
            rep = f"GateAwareFedAMP(expert_threshold={expert_threshold})"
    ...
    def aggregate(self, local_weights: List[Tuple[List[np.ndarray], int]],
              current_model: torch.nn.Module) -> torch.nn.Module:
        aggregated = []
        if self.alg == "fedavg" or self.alg == "fedprox":
            aggregated = fedavg_aggregate(local_weights, current_model)
        elif self.alg == "avg":
            aggregated = simple_aggregate(local_weights, current_model)
        elif self.alg == "medianavg":
            aggregated = median_aggregate(local_weights,current_model)
        elif self.alg == "fednova":
            ...
            # 调用 DDPG 聚合逻辑
            aggregated = aggregate_with_ddpg(self.params["agent"], self.params["client_features"],
                                             self.params["client_models"], current_model)
