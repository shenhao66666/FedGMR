...
class SimpleClientManager(ClientManager):
    ...
    def sample(self, c: float) -> List[ClientProxy]:
        ...
        # sampled_clients = random.sample(available_clients, num_selection)
         # 固定顺序，全部返回
        available_clients.sort()  # 按名字排序，保证每轮顺序一致
        num_selection = len(available_clients)
        log(INFO, f"Parameter c={c}. Sampled {num_selection} client(s): {available_clients}")
        return [self.clients[cid] for cid in available_clients]
