...
def log_metrics(y_true: np.ndarray,
    ...
    for dim in range(shape):
        ...
        nrmse = rmse / (np.mean(np.abs(y_true_dim)) + 1e-8)
...
def accumulate_metric(y_true: Union[np.ndarray, torch.tensor],
    ...
    eps = 1e-8  # 防止除以0
    # nrmse_first_dim = rmse_first_dim / (np.mean(y_true_first_dim) + eps)
    denom_first_dim = np.mean(np.abs(y_true_first_dim)) + eps
    nrmse_first_dim = rmse_first_dim / denom_first_dim
    ...
    if y_true.shape[1] >= 2:
        ...
        for i in range(1, len(dims)):
            ...
            denom_dim = np.mean(np.abs(y_true_dim)) + eps
            nrmse_dim = rmse_dim / denom_dim
            nrmses += nrmse_dim
